...
class TestSearch:
    ...
    def test_ok(self):
        ...
        code,pages=self.buyer.search(self.buyer_id,'title',book.title)
        assert code == 200 and pages == 0
        # assert book.id in bids and pages == 0
    
    def test_not_in(self):
        book:Book=self.buy_book_info_list[0][0]
        code,pages=self.buyer.search(self.buyer_id,'title',book.title+"_x")
        assert code == 200 and pages == 0
        # assert len(bids) == 0 and pages == 0
        
    def test_next_and_pre(self):
        code,pages=self.buyer.search(self.buyer_id,'tags', '\n')
        assert code == 200 and pages > 0
        book_id = bids[0]
        code,bids1,page=self.buyer.next_page(self.user_id, 0, pages)
    ...
    def test_specific_page(self):
        code,bids,pages=self.buyer.search(self.buyer_id,'tags', '\n')
        assert code == 200 and pages > 0
        book_id = bids[0]
    ...
    def test_non_next_and_pre_and_specific(self):
        book:Book=self.buy_book_info_list[0][0]
        code,bids,pages=self.buyer.search(self.buyer_id,'title',book.title)
    ...
    def test_partial_name(self):
        book:Book=self.buy_book_info_list[0][0]
        code,bids,pages=self.buyer.search(self.buyer_id,'title',book.title[0])
        assert code == 200
        assert book.id in bids

    def test_wrong_keyword(self):
        book:Book=self.buy_book_info_list[0][0]
        code,bids,pages=self.buyer.search(self.buyer_id,'wrong',book.title[0])
        assert code != 200
    ...
    def test_specific_store(self):
        ...
        code,bids,pages = self.buyer.search(self.buyer_id,'title',book.title,store_id=self.store_id)
        assert code == 200 and book.id in bids and pages == 0
        code,bids,pages = self.buyer.search(self.buyer_id,'title',book.title,store_id=store_id)
        assert code == 200 and not book.id in bids and pages == 0

    def test_wrong_store_id(self):
        book:Book=self.buy_book_info_list[0][0]
        code,bids,pages=self.buyer.search(self.buyer_id,'title',book.title,store_id=self.store_id+'_x')
        assert code != 200
    ...
    def test_zero_stock(self):
        ...
        code,bids,pages=self.buyer.search(self.buyer_id,'title','abc',store_id=store_id)
