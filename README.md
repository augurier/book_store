...
学号：10225501448
## 1. 开发流程
在pj1的开发过程中，曾经有一个版本是在sqlite数据库下实现了大部分功能。因此我先使用了版本回退找到了该版本，在此基础上补全了新开发的功能，调整数据库设计，转移数据库至postgresql并进行进一步的开发。  
...
虽然实验要求中说可以将blob数据分离存储，但在实际实现中我最终选择将所有书本信息仍旧存储在mongodb中。这主要是出于以下考量：  
...
综上，我选择仍然使用nosql存储书本信息。book_id作为关联使用的约定外键。在实现过程中我同样将书本信息存储在sql中进行尝试比较，存储代码文件为bookstore/fe/data/save_psql.py。  
...
### 3.3 搜索图书
对于搜索图书，我采用了分页设计，同时基于性能和业务需求的考量（具体见pj1 4.3部分），设计了两步查询的查询逻辑。而由于这里独特的sql和nosql结合的数据库设计，在具体实现时也将两种语言相结合：需要书本具体信息进行模糊查询时采用nosql，得到查询结果book_id时存入user_表，以便后续分页操作。（这里的细节处理是，把原本数组形式的book_id用逗号分隔合成字符串，以便存入关系数据库。）  
具体实现如下：  
```python
...
## 6. 版本控制
这里主要操作是pj1的版本回退和进一步开发。由于这次的项目是单人实现的，阶段实现的代码简单存放在了本地暂存区，全部完成后一并推送到了仓库。仓库地址是 https://github.com/augurier/book_store，保存了两个版本的分支 。
